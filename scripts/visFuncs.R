...
visPlates <- function(dir){
  ...
  # # open all files, then calibrate range based on global range
  # lapply(files, function(x) 
  #   lapply(x, function(y) as.numeric(as.matrix(read.csv(y))))) -> range
  # as.numeric(unlist(range)) -> range
  # min(range) -> min.r
  # max(range) -> max.r
  # mean(range)+2*sd(range) -> heatmap.r
